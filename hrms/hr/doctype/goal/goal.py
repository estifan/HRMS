...
class Goal(NestedSet):
    ...
    def on_update(self):
        ...
        # No longer updating progress here, handled by the standalone method
    ...
    def validate_from_to_dates(self, start_date, end_date):
        ...
        if self.parent_goal:
            ...
            if parent_goal.start_date and parent_goal.start_date > start_date:
                frappe.throw(_("Goal start date cannot be before parent goal's start date"))

            if parent_goal.end_date and parent_goal.end_date < end_date:
                frappe.throw(_("Goal end date cannot be after parent goal's end date"))
